use design_reasoning::{LanguageEngine, LanguageState};
...
pub fn verbalize_candidate(input: &str, c: &SavedCandidate) -> String {
    ...
    let explanation = engine.explain_state(&state);

    let mut out = String::new();
    out.push_str(&explanation.summary);
    out.push('\n');

    // TemplateId の説明文を付加（stability/ambiguity から選択）
...
fn build_language_state(input: &str, c: &SavedCandidate) -> LanguageState {
    ...
    let stability_score =
        (c.evaluation.structural_quality * 0.6 + c.evaluation.dependency_quality * 0.4).clamp(0.0, 1.0);
...
mod tests {
    ...
    fn make_candidate(structural: f64, dependency: f64, complexity: f64) -> SavedCandidate {
        SavedCandidate {
            ...
            components: vec!["service_1".to_string(), "service_2".to_string(), "database_1".to_string()],
    ...
    fn test_verbalize_unstable_contains_warning() {
        ...
        assert!(result.contains("不安定") || result.contains("再定義"), "should warn about instability");
    }
