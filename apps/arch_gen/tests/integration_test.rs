...
//! バイナリを `cargo run -p arch_gen --` 経由で呼び出し、
...
/// `cargo run -p arch_gen -- <args>` を実行して (stdout, status) を返す。
fn arch_gen(args: &[&str]) -> (String, bool) {
    ...
    let output = cmd.output().expect("failed to run arch-gen via cargo");
...
fn test_generate_basic() {
    ...
    assert!(success, "arch-gen generate should exit 0\nstdout: {stdout}");
    assert!(
        stdout.contains("Candidate") || stdout.contains("Architecture"),
        "stdout should contain candidate output\n{stdout}"
    );
    assert!(
        stdout.contains("EvaluationVector"),
        "stdout should contain EvaluationVector\n{stdout}"
    );
}
...
fn test_generate_mermaid_format() {
    let (stdout, success) = arch_gen(&[
        "generate", "マイクロサービスを設計する",
        "--no-code", "-n", "1", "-f", "mermaid",
    ]);
    assert!(success);
    assert!(stdout.contains("graph TD"), "mermaid output should contain 'graph TD'\n{stdout}");
}

#[test]
fn test_generate_markdown_format() {
    let (stdout, success) = arch_gen(&[
        "generate", "イベント駆動システムを設計する",
        "--no-code", "-n", "1", "-f", "markdown",
    ]);
    assert!(success);
...
fn test_generate_plantuml_format() {
    let (stdout, success) = arch_gen(&[
        "generate", "シンプルなWebアプリを設計する",
        "--no-code", "-n", "1", "-f", "plantuml",
    ]);
    assert!(success);
    assert!(stdout.contains("@startuml"), "plantuml output should contain '@startuml'\n{stdout}");
...
fn test_evaluate_and_export_from_file() {
    ...
    // export mermaid
    let (merm_out, merm_ok) = arch_gen(&[
        "export", design_path.to_str().unwrap(), "-f", "mermaid",
    ]);
    ...
    let (_, md_ok) = arch_gen(&[
        "export", design_path.to_str().unwrap(),
        "-f", "markdown",
        "-o", md_path.to_str().unwrap(),
    ]);
...
fn test_explain_from_generated_file() {
    ...
    let (_, ok) = arch_gen(&[
        "generate", "マイクロサービスAPIを設計する",
        "--no-code", "-n", "1", "-o", out_dir,
    ]);
    assert!(ok);
...
fn test_refine_produces_refined_json() {
    ...
    assert!(refined_path.exists(), "design_refined.json should be created");
}
...
fn test_generate_file_reference() {
    ...
    let req_path = repo_root()
        .join("examples/requirements/simple_webapp.txt");
    if req_path.exists() {
