...
fn run(cfg: BatchConfig) -> Result<(), String> {
    ...
    for &seed in &cfg.seeds {
        ...
        let integrity = step0::validate_cases(&generated_cases, seed)
            .map_err(|errs| format!("Step0 generator gate failed (seed={seed}): {}", errs.join("; ")))?;
        ...
        let sampled = raw_rows.into_iter().take(cfg.cases.min(loaded_cases.len())).collect::<Vec<_>>();
    ...
    let summary_json =
        serde_json::to_string_pretty(&summary).map_err(|e| format!("summary serialize failed: {e}"))?;
