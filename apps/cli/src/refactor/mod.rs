...
pub struct CandidateFromModuleInput {
    pub kind: RefactorActionKind,
    pub title: String,
    pub rationale: String,
    pub target: RefactorTarget,
...
pub fn candidate_from_module(
    report: &AnalysisReport,
    logical_name: &str,
    input: CandidateFromModuleInput,
) -> RefactorCandidate {
    let CandidateFromModuleInput {
        kind,
        title,
        rationale,
        target,
    ...
    match target {
        RefactorTarget::Cycle => candidate_from_module(
            ...
            report
                ...
                .unwrap_or("core"),
            CandidateFromModuleInput {
                kind: RefactorActionKind::ExtractInterface,
                title: "Cycle remediation".to_string(),
                rationale: "Break the dominant cycle via interface extraction".to_string(),
                target: target.clone(),
        ...
        RefactorTarget::ExtractInterface { from, to } => candidate_from_module(
            ...
            CandidateFromModuleInput {
                kind: RefactorActionKind::ExtractInterface,
                title: format!("Extract interface {from} -> {to}"),
        ...
        RefactorTarget::LayerViolation(detail) => candidate_from_module(
            ...
            CandidateFromModuleInput {
                kind: RefactorActionKind::RemoveDependency,
                title: format!("Fix layer violation {detail}"),
                rationale: "Stabilize layer direction before apply".to_string(),
                target: target.clone(),
                target_nodes: vec![detail.clone()],
                target_edges: Vec::new(),
        ...
        RefactorTarget::RenameBoundary(module) => candidate_from_module(
            ...
            CandidateFromModuleInput {
                ...
                title: format!("Rename boundary {module}"),
                rationale: "Clarify boundary naming".to_string(),
                target: target.clone(),
                target_nodes: vec![module.clone()],
                target_edges: Vec::new(),
        ...
        RefactorTarget::IntroduceService(module) => candidate_from_module(
            ...
            CandidateFromModuleInput {
                ...
                title: format!("Introduce service for {module}"),
                rationale: "Move orchestration into a service node".to_string(),
                target: target.clone(),
                target_nodes: vec![module.clone()],
                target_edges: Vec::new(),
        ...
        RefactorTarget::FileMove(path) => candidate_from_module(
            ...
            CandidateFromModuleInput {
                kind: RefactorActionKind::MoveFile,
                title: format!("Move file {}", path.display()),
                rationale: "Move file to a new boundary".to_string(),
                target: target.clone(),
                target_nodes: vec![path.display().to_string()],
                target_edges: Vec::new(),
