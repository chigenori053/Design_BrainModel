    ...
    pub fn evaluate_execution(
        ...
        if self.recursion_guard.increment().is_err() {
            ...
            return self.create_decision(
                ...
                capability,
                rollback_ready,
                projection.risk_level.clone(),
                explanation,
                RuntimeExecutionState::SafeMode,
            );
        }

        let (approved, ja_msg, en_msg, state) =
            if capability == ExecutionCapability::SelfModification {
                (
                    true,
                    "自己変更実行中のためランタイムを隔離モードへ移行しました。",
                    "The runtime entered isolation mode during self-modification.",
                    RuntimeExecutionState::IsolationMode,
                )
            } else if !rollback_ready && capability != ExecutionCapability::ReadOnly {
                (
                    false,
                    "安全なロールバックを保証できないため実行は停止されました。",
                    "Execution was blocked because safe rollback recovery could not be guaranteed.",
                    RuntimeExecutionState::ExecutionRejected,
                )
                ...
                (
                    ...
                    RuntimeExecutionState::ExecutionSuspended,
                )
            } else if projection.risk_level == WorkspaceRiskLevel::High {
                (
                    false,
                    "長期的な整合性リスクが検出されたため実行は拒否されました。",
                    "Execution was rejected due to long-term semantic consistency risks.",
                    RuntimeExecutionState::ExecutionRejected,
                )
            } else if projection.governance_required {
                (
                    false,
                    "実行統治の検証に失敗したため安全状態へ移行しました。",
                    "Governance validation failed and the runtime entered safe mode.",
                    RuntimeExecutionState::SafeMode,
                )
            } else {
                (
                    true,
                    "実行統治裁定により変更が許可されました。",
                    "The modification was approved through governance arbitration.",
...
mod tests {
    ...
    fn test_recursion_protection() {
        let mut engine = ExecutionPermitEngine {
            recursion_guard: GovernanceRecursionGuard {
                current_depth: 3,
                max_depth: 3,
            },
        };
        ...
        assert_eq!(
            ...
            "Governance recursion depth limit exceeded."
        );
    }
    ...
    fn test_self_modification_isolation() {
        ...
        let proj = test_projection(WorkspaceRiskLevel::Minimal, true, false);
        let decision =
            engine.evaluate_execution(&proj, ExecutionCapability::SelfModification, true);
