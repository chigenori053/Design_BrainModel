#[path = "contract/coding_apply_path_resolution_contract.rs"]
mod coding_apply_path_resolution_contract;
#[path = "contract/error_contract.rs"]
mod error_contract;
#[path = "contract/graph_node_source_binding_contract.rs"]
mod graph_node_source_binding_contract;
#[path = "contract/gui_viewer_contract.rs"]
mod gui_viewer_contract;
#[path = "contract/nl_autonomous_goal_contract.rs"]
mod nl_autonomous_goal_contract;
#[path = "contract/nl_interface_contract.rs"]
mod nl_interface_contract;
#[path = "contract/nl_multiturn_contract.rs"]
mod nl_multiturn_contract;
#[path = "contract/onboarding_help_contract.rs"]
mod onboarding_help_contract;
#[path = "contract/qualified_module_identity_contract.rs"]
mod qualified_module_identity_contract;
#[path = "contract/version_contract.rs"]
mod version_contract;
